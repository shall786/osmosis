...
func (s *IntegrationTestSuite) TestConcentratedLiquidity() {
	...
	addr1PosId := node.CreateConcentratedPosition(address1, "[-1200]", "400", fmt.Sprintf("10000000%s", denom0), fmt.Sprintf("10000000%s", denom1), 0, 0, freezeDuration.String(), poolID)
	node.CreateConcentratedPosition(address1, "[-400]", "400", fmt.Sprintf("10000000%s", denom0), fmt.Sprintf("10000000%s", denom1), 0, 0, freezeDuration.String(), poolID)

	// Create 1 position for address2: does not overlap with anything, ends at maximum
	addr2PosId := node.CreateConcentratedPosition(address2, "2200", fmt.Sprintf("%d", maxTick), fmt.Sprintf("10000000%s", denom0), fmt.Sprintf("10000000%s", denom1), 0, 0, freezeDuration.String(), poolID)

	// Create 2 positions for address3: overlap together, overlap with 2 address1 positions, one position starts from minimum
	addr3PosId := node.CreateConcentratedPosition(address3, "[-1600]", "[-200]", fmt.Sprintf("10000000%s", denom0), fmt.Sprintf("10000000%s", denom1), 0, 0, freezeDuration.String(), poolID)
	node.CreateConcentratedPosition(address3, fmt.Sprintf("[%d]", minTick), "1400", fmt.Sprintf("10000000%s", denom0), fmt.Sprintf("10000000%s", denom1), 0, 0, freezeDuration.String(), poolID)
	...
	sqrtPriceAfterNextInitializedTick, err := cl.TickToSqrtPrice(nextInitTick.Add(tickOffset), sdk.NewInt(precisionFactorAtPriceOne))
	s.Require().NoError(err)
	sqrtPriceAtNextInitializedTick, err := cl.TickToSqrtPrice(nextInitTick, sdk.NewInt(precisionFactorAtPriceOne))
	s.Require().NoError(err)
	...
	sqrtPricebBelowNextInitializedTick, err := cl.TickToSqrtPrice(nextInitTick.Sub(tickOffset), sdk.NewInt(precisionFactorAtPriceOne))
	s.Require().NoError(err)
	sqrtPriceAtNextInitializedTick, err = cl.TickToSqrtPrice(nextInitTick, sdk.NewInt(precisionFactorAtPriceOne))
	s.Require().NoError(err)
