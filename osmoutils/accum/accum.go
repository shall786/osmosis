...
func (accum AccumulatorObject) NewPositionCustomAcc(name string, numShareUnits sdk.Dec, customAccumulatorValue sdk.DecCoins, options *Options) error {
	...
	return initOrUpdatePosition(accum, customAccumulatorValue, name, numShareUnits, sdk.NewDecCoins(), options)
}
...
func (accum AccumulatorObject) AddToPositionCustomAcc(name string, newShares sdk.Dec, customAccumulatorValue sdk.DecCoins) error {
	...
	if err := validateAccumulatorValue(accum.value, position.InitAccumValue.Add(customAccumulatorValue...)); err != nil {
		return err
	}
	...
	return initOrUpdatePosition(accum, customAccumulatorValue, name, oldNumShares.Add(newShares), unclaimedRewards, position.Options)
}
...
func (accum AccumulatorObject) RemoveFromPositionCustomAcc(name string, numSharesToRemove sdk.Dec, customAccumulatorValue sdk.DecCoins) error {
	...
	return initOrUpdatePosition(accum, customAccumulatorValue, name, oldNumShares.Sub(numSharesToRemove), unclaimedRewards, position.Options)
}
...
func (accum AccumulatorObject) ClaimRewardsCustomAcc(positionName string, customAccumulatorValue sdk.DecCoins) (sdk.Coins, error) {
	...
	totalRewards := getTotalRewards(accum, position, customAccumulatorValue)
	...
	if position.NumShares.Equal(sdk.ZeroDec()) {
		...
		err := initOrUpdatePosition(accum, accum.value.Sub(customAccumulatorValue), positionName, position.NumShares, sdk.NewDecCoins(), position.Options)
		if err != nil {
			return sdk.Coins{}, err
		}
	}
