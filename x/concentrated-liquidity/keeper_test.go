...
var (
	...
	SmallCoin0                                     = sdk.NewCoin(ETH, sdk.NewInt(10000))
	SmallCoin1                                     = sdk.NewCoin(USDC, sdk.NewInt(5000000))
	LargeCoin0                                     = sdk.NewCoin(ETH, sdk.NewInt(1000000000000000000))
	LargeCoin1                                     = sdk.NewCoin(USDC, sdk.NewInt(1000000000000000000))
)
...
func (s *KeeperTestSuite) chargeFeesToPool(ctx sdk.Context, pool types.ConcentratedPoolExtension, owner sdk.AccAddress) (feeCharged sdk.Coin) {
	swapCoin0, swapCoin1 := sdk.NewCoin(ETH, sdk.NewInt(5000000000000000000)), sdk.NewCoin(USDC, sdk.NewInt(5000000000000000000))
	s.FundAcc(owner, sdk.NewCoins(swapCoin0, swapCoin1))
	_, _, _, err := s.App.ConcentratedLiquidityKeeper.CreatePosition(ctx, pool.GetId(), owner, DefaultCoin0.Amount, DefaultCoin1.Amount, sdk.ZeroInt(), sdk.ZeroInt(), DefaultLowerTick, DefaultUpperTick, ctx.BlockTime().Add(DefaultFreezeDuration))
	s.Require().NoError(err)
	coinIn, _, _, _, _, err := s.App.ConcentratedLiquidityKeeper.SwapOutAmtGivenIn(s.Ctx, DefaultCoin0, USDC, pool.GetSwapFee(s.Ctx), cltypes.MinSpotPrice, pool.GetId())
	s.Require().NoError(err)
	fee := coinIn
	fmt.Println("fee", fee)
	fee.Amount = fee.Amount.ToDec().Mul(pool.GetSwapFee(s.Ctx)).TruncateInt()
	fmt.Println("fee.Amount", fee.Amount)
	feeCharged = fee
	return feeCharged
}
